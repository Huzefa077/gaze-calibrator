// Shared names for the conceptual lifecycle. VisionTracker currently owns the
// rendered phase state, while these exports remain useful to session consumers.
...
// A reusable, evenly spaced 5x5 grid expressed as viewport percentages.
...
// Creates a plain in-memory session record. Calling this does not start the
// camera or WebGazer; it only returns initial application data.
...
// Returns a new session containing one more calibration sample. The input
// object is not mutated, which makes it safe to store in React state.
